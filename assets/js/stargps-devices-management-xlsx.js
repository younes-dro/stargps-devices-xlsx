...
//                    console.log( textStatus + " :  " + response.status + " : " + errorThrown );
		...
		$('#stargps_device_management_date_recharge').click(function (e) {
			...
			$.ajax({
				...
				type: "POST",
				context: this,
				data: { 'action': 'stargps_device_management_devices_date_recharge_xlsx' ,'date_recharge': $('input[name=DateOfRecharge]').val(),'next_recharge': $('input[name=NextOfRecharge]').val(), 'customer_name': $('#customer_name').val(), 'type_device': $('#type_device').val(), 'app': $('#app').val(), 'imei': $('#imei').val(), 'stargps_device_management_nonce': starGPSDevicesManagementXlsxParams.stargps_device_management_nonce },
				beforeSend: function () {
